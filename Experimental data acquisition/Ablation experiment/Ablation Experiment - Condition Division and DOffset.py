...
def visualize_realtime_hi_median_filtered(hi_realtime, hi_realtime_filtered, save_path=None):

    fig = plt.figure(figsize=(18, 6))
    ax = fig.add_subplot(111)
    ax.plot(hi_realtime, color='#9ecae1', linewidth=1.0, alpha=0.65,
            label='Real-time HI (Kalman Filtered)')
    ax.plot(hi_realtime_filtered, color='#08519c', linewidth=2.0,
            label='Real-time HI (Median Filtered)')
    ax.axhline(y=1.0, color='green', linestyle='--', alpha=0.6)
    ax.axhline(y=0.3, color='red', linestyle=':', alpha=0.6)
    ax.set_title('Real-time Health Indicator with Median Filtering')
    ...
    if save_path is not None:
        ...
        print(f"Real-time HI filtered figure saved to: {save_path}")
    plt.show()
...
def main():
    ...
    # 仅保存未融合 HI，不输出累计 HI、融合 HI、融合置信度及融合权重
    ...
    origin_hi_df.to_csv(os.path.join(final_output_dir, "Origin_HI_Results.csv"), index=False)
